...
gc = gspread.service_account(filename="MyLogins.json")

# Open Google Sheet using its URL
sh = gc.open_by_url("https://docs.google.com/spreadsheets/d/1oW4E5NPQTM0DXf1Clp3JiT5ljVfCWfSOxVM7DJn7Iyk/edit?usp=sharing")
...
# Extract from CSV
csv_data = pd.read_csv("Diseases.csv")
...
# sql_data = standardize_columns(sql_data)
# api_data = standardize_columns(api_data)
...
# Convert date column to datetime format and align to UTC
...
# sql_data = aggregate_data(sql_data)
# api_data = aggregate_data(api_data)
...
final_data.to_csv("Processed_cleaned_data.csv", index=False)


print("ETL pipeline succeeded. Cleaned data saved as Processed_cleaned_data.csv")
